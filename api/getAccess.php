...
if (isset($_GET["code"])) {
    ...
    $userInfo = $sql->query("select Id,openid,refresh from `user` where `openid` = '$access[openid]'")->fetch_array();
    if ($userInfo) {
        $expire = $sql->query("select ACT_expires from `user` where Id = $userInfo[Id]")->fetch_row()[0];
        if ($expire <= time()) {
            ...
            $sql->query("update `user` set `sessionId` = '$access[access_token]',ACT_expires = '$time',`refresh`='$access[refresh_token]' WHERE Id = '$userInfo[0]'");
        }
    } else {
        $url = "https://api.weixin.qq.com/sns/userinfo?access_token=$access[access_token]&openid=$access[openid]&lang=zh_CN";
        $userInfo = json_decode(file_get_contents($url), true);
        $time = time() + $access["expires_in"];
        $sql->query("insert into `user` (`name`,`openid`,`seesionId`,`ACT_expires`,`refresh`,`head`) VALUE ('$userInfo[nickname]','$userInfo[openid]','$access[access_token]','$time','$access[refresh_token]','$userInfo[headimgurl]')");
